...
export function DocumentUploadModal({ patient, initialCategory = 'other', onClose, onSaved }: { patient: FamilyCarePatient; initialCategory?: 'lab' | 'other'; onClose: () => void; onSaved: (message: string) => void }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const saving = Math.max(0, Math.round((1 - result.storedBytes / result.sourceBytes) * 100));
      onSaved(saving ? `Documento guardado y comprimido. Usa ${saving}% menos espacio.` : 'Documento guardado correctamente; ya estaba optimizado.');
    } catch (reason) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="profile-modal compact-modal" role="dialog" aria-modal="true" aria-labelledby="document-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <p className="profile-intro">Antes de subirlo se optimiza para reducir espacio: imágenes a WebP y PDFs con compresión sin pérdida cuando conviene.</p>
      <form onSubmit={save}>
        ...
        <div className="data-caution"><strong>Conservación clínica</strong><span>La copia guardada sigue siendo legible y descargable. Los PDF se comprimen sin alterar su contenido.</span></div>
