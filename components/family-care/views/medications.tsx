...
export function Medications({ patients, patient, onNotice, onRegister, onChanged, revision }: {
    ...
    useEffect(() => {
        ...
        loadCarePlans(targets).then((value) => { if (active)
            ...
            if (active) {
                ...
                onNotice('No fue posible cargar los medicamentos.', 'warning');
            }
        });
        return () => { active = false; };
    ...
    {plans && medications.length > 0 && <section className="card-list medication-list">{medications.map((medication, index) => {
        ...
        <div className="medication-main"><p>{patientLabel(medication.patient)} · {medication.active ? 'Tratamiento activo' : 'Finalizado'}{medication.source === 'healwave' ? ' · Healwave' : ''}</p><h2>{medication.name}</h2><span>{[medication.doseText, medication.route, medication.instructions].filter(Boolean).join(' · ') || 'Sin instrucciones adicionales'}</span>{medication.prescribedBy && <small>Indicado por {medication.prescribedBy}</small>}
