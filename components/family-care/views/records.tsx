...
export function Records({ profile, patient, onNotice, onShare, onEdit, onRegister, canShare, canEdit, revision }: {
    profile: string;
    ...
    const birthDate = normalizeDateOnly(patient?.birthDate);
    useEffect(() => {
        if (!patient)
            return;
        ...
        fetchClinicalRecords(patient.id, controller.signal).then((records) => setLoaded({ patientId: patient.id, records })).catch((error: unknown) => {
            if (!(error instanceof DOMException && error.name === 'AbortError'))
                onNotice('No fue posible cargar el expediente.', 'warning');
        });
        return () => controller.abort();
    }, [patient, revision, onNotice]);
    const timeline = useMemo(() => [
        ...records.encounters.map((entry) => ({ id: `encounter-${entry.id}`, at: entry.occurredAt, kind: entry.encounterType === 'hospitalization' ? 'hospitalization' : 'consultation', title: entry.reason, meta: [entry.specialty, entry.practitionerName, entry.facilityName].filter(Boolean).join(' · ') || encounterLabels[entry.encounterType], copy: entry.summary || 'Sin resumen adicional.', tag: entry.source === 'healwave' ? `${encounterLabels[entry.encounterType] || 'Atención'} · Healwave` : encounterLabels[entry.encounterType] || 'Atención' })),
        ...records.labReports.map((report) => ({ id: `lab-${report.id}`, at: report.collectedAt, kind: 'lab', title: report.panelName, meta: [report.laboratoryName, `${report.results.length} resultados`].filter(Boolean).join(' · '), copy: report.results.slice(0, 4).map((result) => `${result.analyteName}: ${result.valueNumeric ?? result.valueText} ${result.unit || ''}`.trim()).join(' · '), tag: report.source === 'healwave' ? 'Laboratorio · Healwave' : 'Laboratorio' })),
