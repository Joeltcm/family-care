...
function size(bytes: number) {
    if (!bytes)
        return 'Tamaño original';
    if (bytes < 1024 * 1024)
        return `${Math.max(1, Math.round(bytes / 1024))} KB`;
    return `${(bytes / 1024 / 1024).toFixed(1)} MB`;
}
export function Documents({ patient, canEdit, onUpload, onNotice, revision }: {
    ...
    useEffect(() => {
        ...
        fetchClinicalRecords(patient.id, controller.signal).then((records) => setLoaded({ patientId: patient.id, records })).catch((error: unknown) => {
            if (!(error instanceof DOMException && error.name === 'AbortError'))
                onNotice('No fue posible cargar los documentos.', 'warning');
        });
