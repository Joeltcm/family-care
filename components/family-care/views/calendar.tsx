...
export function Calendar({ patients, patient, onNotice, onRegister, onChanged, revision }: {
    ...
    useEffect(() => {
        ...
        if (!targets.length)
            return () => { active = false; };
        loadCarePlans(targets).then((value) => { if (active)
            setLoaded({ key: targetKey, plans: value }); }).catch(() => {
            if (active) {
                setLoaded({ key: targetKey, plans: [] });
                onNotice('No fue posible cargar el calendario.', 'warning');
            }
        });
        return () => { active = false; };
    ...
    async function changeStatus(item: Appointment & {
        patient: FamilyCarePatient;
