import { callFamilyCareApi, sessionCookie } from '@/lib/server/family-care-api';
...
export async function POST(request: Request) {
  ...
  try {
    const response = await callFamilyCareApi(request, '/v1/auth/setup-password', {
    ...
    return Response.json({ error: payload.error || 'password_setup_failed' }, { status: response.status, headers });
  } catch {
