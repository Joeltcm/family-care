...
export async function POST(request: Request, context: { params: Promise<{ patientId: string; medicationId: string }> }) {
  if (request.headers.get('origin') !== new URL(request.url).origin) return Response.json({ error: 'invalid_origin' }, { status: 403 });
  try {
    const { patientId, medicationId } = await context.params;
    const response = await callFamilyCareApi(request, `/v1/patients/${encodeURIComponent(patientId)}/medications/${encodeURIComponent(medicationId)}/events`, {
