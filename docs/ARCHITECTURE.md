...
- La PWA autentica correo y contraseña contra Railway mediante un puente servidor a servidor. La contraseña usa Argon2id; el navegador recibe solo una cookie `HttpOnly` con un token opaco cuyo digest se guarda en PostgreSQL. Sites y Cloudflare Access se conservan como puentes heredados durante la transición, pero no son obligatorios para familiares invitados. El cliente nunca puede escoger su identidad.
